...
@pytest.mark.asyncio
async def test_horizon_filter_near_returns_10_rows(client: AsyncClient, settings, factory) -> None:
...
@pytest.mark.asyncio
async def test_horizon_filter_med_returns_10_rows(client: AsyncClient, settings, factory) -> None:
    recompute_scores.run(settings=settings, factory=factory)
...
@pytest.mark.asyncio
async def test_horizon_filter_long_returns_10_rows(client: AsyncClient, settings, factory) -> None:
    recompute_scores.run(settings=settings, factory=factory)
...
@pytest.mark.asyncio
async def test_horizon_filter_omitted_returns_30(client: AsyncClient, settings, factory) -> None:
    """Backwards compat: no ?horizon= param still returns all 30 rows."""
    recompute_scores.run(settings=settings, factory=factory)
